  ...
  <Popover class="relative bg-white">
    <div class="mx-auto max-w-7xl px-4 sm:px-6">
      <div class="flex items-center justify-between border-b-2 border-gray-100 py-6 md:justify-start md:space-x-10">
        ...
        <PopoverGroup as="nav" class="hidden space-x-12 md:flex">
          <router-link to="/" class="text-base font-medium text-gray-500 hover:text-gray-900 items-stretch">首页</router-link>

          <Popover class="relative" v-slot="{ open }">
            <PopoverButton :class="[open ? 'text-gray-900' : 'text-gray-500', 'group inline-flex items-center rounded-md bg-white text-base font-medium hover:text-gray-900 focus:outline-none']">
              <span>新闻</span>
        ...
        <div class="hidden items-center justify-end md:flex md:flex-1 lg:w-0">
          <button class="whitespace-nowrap text-base font-medium text-gray-500 hover:text-gray-900" @click="showGuard">登录</button>
          <Guard
              :appId="appId"
              :visible="visible"
              :config="config"
              ...
              @close="onClose"
          ></Guard>
          <button class="ml-8 inline-flex items-center justify-center whitespace-nowrap rounded-md border border-transparent bg-green-700 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-green-800">注册</button>
        </div>
      </div>
    </div>
        ...
        <div class="divide-y-2 divide-gray-50 rounded-lg bg-white shadow-lg ring-1 ring-black ring-opacity-5">
          <div class="px-5 pt-5 pb-6">
            <div class="flex items-center justify-between">
              <div>
                <img class="h-8 w-auto" src="../../../assets/logo.png" alt="知行通" />
              </div>
              <div class="-mr-2">
                <PopoverButton class="inline-flex items-center justify-center rounded-md bg-white p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none">
          ...
          <div class="space-y-6 py-6 px-5">
            ...
            <div>
              <button class="flex w-full items-center justify-center rounded-md border border-transparent bg-green-700 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-green-800">注册</button>
              <p class="mt-6 text-center text-base font-medium text-gray-500">
                Existing customer?
                {{ ' ' }}
                <button class="text-green-600 hover:text-green-800" @click="showGuard">登录</button>
              </p>
            </div>
          </div>
        </div>
...
const news = [
  {
    name: '校园新闻',
    description: '不仅发布学校官方资讯，也记录着校园的美好瞬间',
    href: '/schoolnews',
    icon: NewspaperIcon,
  },
  {
    name: '竞赛资讯',
    description: '获取最新的大学生权威竞赛信息',
    href: '/gamenews',
    icon: CursorArrowRaysIcon,
  }
]

const appId = ref('6340e8a277da823f8a5970cc')
const config = ref({mode: "modal", contentCss: ".g2-modal-close {margin-left: 10px;}"})
